...
s32 Macbeth_801A0308(Actor*, f32, f32, u8);
...
s32 Macbeth_MaTenderCar_OverrideLimbDraw(s32, Gfx**, Vec3f*, Vec3f*, void*);
...
void Macbeth_RotateTrainWheels(void) {
    ...
    gfx_texture_cache_invalidate(D_MA_6023228);
    gfx_texture_cache_invalidate(D_MA_6023388);
}

void Macbeth_TrainCarSpeed_Update(Actor* this) {
...
s32 Macbeth_MaLocomotive_HandleDamage(MaLocomotive* this) {
...
void Macbeth_MaBoulderCar_Update(MaBoulderCar* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
...
void Macbeth_MaRobot_Update(MaRobot* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
...
void Macbeth_MaContainerBox_Update(MaContainerBox* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
...
void Macbeth_MaMissileCar_Update(Actor* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
...
void Macbeth_MaTankCar_Update(Actor* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
...
void Macbeth_8019F164(MaCannonCar* this) {
            ...
            if (((this->dmgType != DMG_NONE) && (this->dmgPart >= arg3) && (arg4 >= this->dmgPart)) ||
                ...
                *arg2 -= this->damage;
                if ((*arg2 <= 0) || ((D_MA_801BE2F0[1] - 1 == this->iwork[5]) && (this->iwork[5] < D_MA_801BE2F0[3])) ||
                    (((D_MA_801BE2F0[1] + 1) == this->iwork[5]) && (D_MA_801BE2F0[3] < this->iwork[5]))) {
                    gHitCount++;
                    if ((this->obj.id == OBJ_ACTOR_MA_BOULDER_CAR) && (this->iwork[3] < 2)) {
                        gHitCount += 5;
                    }
                    (*arg1)++;
                }
...
void Macbeth_MaCannonCar_Update(MaCannonCar* this) {
    switch (this->state) {
        case 0:
            Macbeth_TrainCarSpeed_Update(this);
            Macbeth_801A015C(this);
        ...
        if (!Macbeth_801A0308(this, this->obj.pos.z - 420.0f, this->fwork[22], 1)) {
            ...
            if (!Macbeth_801A0308(this, this->obj.pos.z + 420.0f, this->fwork[24], 2)) {
                ...
                if ((this->fwork[19] - this->obj.pos.x) < this->fwork[21]) {
                    this->fwork[25] = this->fwork[23] + ((this->fwork[21] - this->fwork[23]) * 0.5f);
                } else {
                    this->fwork[25] = this->fwork[21] + ((this->fwork[23] - this->fwork[21]) *0.5f);
                }

                if (this->fwork[7] < this->fwork[6]) {
                    this->fwork[8] = this->fwork[7] + ((this->fwork[6] - this->fwork[7]) *0.5f);
                } else {
                    this->fwork[8] = this->fwork[6] + ((this->fwork[7] - this->fwork[6]) *0.5f);
                }

                this->fwork[26] = (this->fwork[22] + this->fwork[24]) *0.5f;
                this->fwork[29] = (this->fwork[27] + this->fwork[28]) *0.5f;
            }
        }
    }
}

s32 Macbeth_801A0308(Actor* this, f32 arg1, f32 arg2, u8 arg3) {
    s16 var_a0 = 0;
    s16 sp4C = 0;
    f32 temp_fv0 = 0;
    f32 sp44 = 0;
...
//    f32 recip_2c_sub_3c = approx_recip_sign(sp2C - sp3C);
...
//        f32 recip_neg_temp_fv0 = -approx_recip_sign(temp_fv0);
  //      f32 recip_2c3cvf02 = approx_recip_sign((sp2C - (sp3C - (temp_fv0 * 2.0f))));
        if (sp24 < 0.3333333f) {
            ...
            this->fwork[19] = (((sp34 - sp44) * sp24) + sp44) -
                              (52.160667f * (1.0f - (1.0f - ((sp2C - arg1) / (sp2C - (sp3C - (temp_fv0 * 2.0f)))))));
        } else {
            this->fwork[19] = ((sp34 - sp44) * sp24) + sp44 +
                              (52.160667f * (1.0f - (1.0f - ((sp2C - arg1)/ (sp2C - (sp3C - (temp_fv0 * 2.0f)))))));
        }
...
s32 Macbeth_801A0A74(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
s32 Macbeth_801A0B00(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
    if (limbIndex == 3) {
        *dList = NULL;
    }
    return 0;
...
s32 Macbeth_801A0B20(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
s32 Macbeth_MaSwitchTrack_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
    MaSwitchTrack* this = (MaSwitchTrack*) thisx;
...
void Macbeth_MaSwitchTrack_PostLimbDraw(s32 limbIndex, Vec3f* rot, void* thisx) {
...
s32 Macbeth_MaLocomotive_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
void Macbeth_Train_Draw(Actor* this) {
    ...
    switch (this->obj.id) {
        case OBJ_ACTOR_MA_CANNON_CAR:
            ...
            // jnmartin84 ????
                ...
                Animation_GetFrameData(&aMaCannonAnim, 0, frameTable);
                Animation_DrawSkeleton(1, aMaCannonSkel, frameTable, Macbeth_801A0B20, NULL, this, &gIdentityMatrix);
            }
            break;
        ...
        case OBJ_ACTOR_MA_BOULDER_CAR:
            if (this->iwork[13] == 0) {
                ...
                if (this->iwork[3] >= 3) {
                    gSPDisplayList(gMasterDisp++, aMaBoulderCarEmptyDL);
                } else {
                    gSPDisplayList(gMasterDisp++, aMaBoulderCarFullDL);
                }
            }
            break;
        ...
        case OBJ_ACTOR_MA_ROBOT:
            if (this->iwork[17] != 2) {
                ...
                gSPDisplayList(gMasterDisp++, aMaRobotDL);
        ...
        case OBJ_ACTOR_MA_CONTAINER_BOX:
            if (this->iwork[17] != 2) {
                ...
                if (this->iwork[17] == 0) {
                    gSPSetGeometryMode(gMasterDisp++, G_CULL_BACK);
                    gSPDisplayList(gMasterDisp++, aMaContainerBoxDL);
                } else {
                    RCP_SetupDL(&gMasterDisp, SETUPDL_57);
                    gSPClearGeometryMode(gMasterDisp++, G_CULL_BACK);
                    gSPDisplayList(gMasterDisp++, aMaContainerBoxDestroyedDL);
                    RCP_SetupDL(&gMasterDisp, SETUPDL_29);
                }
                RCP_SetupDL_29(gFogRed, gFogGreen, gFogBlue, gFogAlpha, gFogNear, gFogFar);
            }
        ...
        case OBJ_ACTOR_MA_TANK_CAR:
            if (this->iwork[13] < 2) {
                ...
                gSPDisplayList(gMasterDisp++, aMaTankCarDL);
                RCP_SetupDL(&gMasterDisp, SETUPDL_29);
            }
            break;
        ...
        case OBJ_ACTOR_MA_MISSILE_CAR:
            if (this->iwork[13] < 2) {
                ...
                if (this->iwork[13] == 0) {
                    Matrix_Translate(gGfxMatrix, 0.0f, -5.0f, 0.0f, MTXF_APPLY);
                    Matrix_SetGfxMtx(&gMasterDisp);
                    gSPDisplayList(gMasterDisp++, aMaMissileCarDL);
                } else {
                    Matrix_Translate(gGfxMatrix, 0.0f, -5.0f, 0.0f, MTXF_APPLY);
                    Matrix_SetGfxMtx(&gMasterDisp);
                    gSPDisplayList(gMasterDisp++, aMaCopperheadMissileDL);
                }
        ...
        case OBJ_ACTOR_MA_LOCOMOTIVE:
            ...
            Animation_DrawSkeleton(1, aMaLocomotiveSkel, frameTable, Macbeth_MaLocomotive_OverrideLimbDraw,
    ...
    id = this->obj.id;
    if ((id == OBJ_ACTOR_MA_ROBOT) || (id == OBJ_ACTOR_MA_CONTAINER_BOX)) {
        if (this->iwork[13] != 2) {
            Matrix_Push(&gGfxMatrix);
...
void Macbeth_TrainTrack_Draw(Scenery* this) {
    ...
    switch (this->obj.id) {
        ...
        case OBJ_SCENERY_MA_SWITCH_TRACK:
            Matrix_Push(&gGfxMatrix);
            Animation_GetFrameData(&aMaSwitchTrackAnim, 0, frameTable);
            Animation_DrawSkeleton(1, aMaSwitchTrackSkel, frameTable, Macbeth_MaSwitchTrack_OverrideLimbDraw,
                                   Macbeth_MaSwitchTrack_PostLimbDraw, this, &gIdentityMatrix);
            Matrix_Pop(&gGfxMatrix);
            ...
            if ((this->state == 1) && (this->timer_4C <= 0)) {
                ...
                Matrix_RotateX(gCalcMatrix, F_PI_2, MTXF_APPLY);
                Matrix_Scale(gGfxMatrix, this->vel.z, this->vel.z * 0.5f, this->vel.z, MTXF_APPLY);
                Matrix_SetGfxMtx(&gMasterDisp);
            ...
            if (D_MA_801BE2F0[5] != 0) {
                ...
                Matrix_Push(&gGfxMatrix);
                Matrix_RotateY(gGfxMatrix, -F_PI_18, MTXF_APPLY);
                Matrix_Translate(gGfxMatrix, 0.0f, 0.0f, -1800.0f, MTXF_APPLY);
                Matrix_SetGfxMtx(&gMasterDisp);
                gSPDisplayList(gMasterDisp++, D_MA_601C170);
                Matrix_Pop(&gGfxMatrix);
...
void Macbeth_MaTower_Draw(MaTower* this) {
    RCP_SetupDL(&gMasterDisp, SETUPDL_57);
    gSPClearGeometryMode(gMasterDisp++, G_CULL_BACK);
...
void Macbeth_MaTrainStopBlock_Draw(MaTrainStopBlock* this) {
    ...
    RCP_SetupDL(&gMasterDisp, SETUPDL_29);
    gSPClearGeometryMode(gMasterDisp++, G_CULL_BACK);
    Animation_GetFrameData(&aMaTrainStopBlockAnim, 0, frameTable);
    Animation_DrawSkeleton(1, aMaTrainStopBlockSkel, frameTable, NULL, Macbeth_MaTrainStopBlock_PostLimbDraw, this,
                           &gIdentityMatrix);
    gSPSetGeometryMode(gMasterDisp++, G_CULL_BACK);
}
...
s32 Macbeth_801A3300(Player* player, f32 arg1, f32 arg2) {
    ...
    sp2C = 1.0f - ((sp30 - arg1) 
    //* approx_recip_sign(sp30 - temp_ft4));
     / (sp30 - temp_ft4));
...
s32 Macbeth_801A3790(Player* player, f32 arg1, f32 arg2) {
...
s32 Macbeth_801A3C20(f32 arg0) {
    ...
    for (var_a0 = D_MA_801BE250[20]; D_i5_801BA138[var_a0].unk_10 != 0; var_a0++) {
        if ((arg0 < D_i5_801BA138[var_a0].unk_00) && ((D_i5_801BA138[var_a0].unk_00 - 1200.0f) < arg0)) {
    ...
    if (sp30 < sp3C) {
        Math_SmoothStepToF(&D_MA_801BE250[23], -10.0f, 0.05f, 1.0f, 0.0f);
    } else if (sp3C < sp30) {
        Math_SmoothStepToF(&D_MA_801BE250[23], 10.0f, 0.05f, 1.0f, 0.0f);
    } else {
        Math_SmoothStepToF(&D_MA_801BE250[23], 0.0f, 0.05f, 1.0f, 0.0f);
    }
    return 1;
}
...
s32 Macbeth_MaRailroadSwitch_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
s32 Macbeth_MaRailwaySignalBase_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
s32 Macbeth_MaRailwaySignalLever_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
    MaRailwaySignal* this = (MaRailwaySignal*) thisx;
...
void Macbeth_MaRailwaySignal_Draw(MaRailwaySignal* this) {
    ...
    Matrix_Push(&gGfxMatrix);
    Animation_GetFrameData(&aMaRailwaySignalLeverAnim, 0, frameTable);
    Animation_DrawSkeleton(1, aMaRailwaySignalLeverSkel, frameTable, Macbeth_MaRailwaySignalLever_OverrideLimbDraw,
                           NULL, this, &gIdentityMatrix);
    Matrix_Pop(&gGfxMatrix);
    Matrix_Push(&gGfxMatrix);
    Animation_GetFrameData(&aMaRailwaySignalBaseAnim, 0, frameTable);
    Animation_DrawSkeleton(1, aMaRailwaySignalBaseSkel, frameTable, Macbeth_MaRailwaySignalBase_OverrideLimbDraw, NULL,
...
void Macbeth_MaProximityLight_Draw(MaProximityLight* this) {
    if ((gPlayer[0].trueZpos - this->obj.pos.z) < this->vel.z) {
        if (gPlayState != PLAY_PAUSE) {
            Math_SmoothStepToF(&this->vel.x, 30.0f, 0.5f, 30.0f, 0.0f);
            if (this->vel.x < 31.0f) {
                this->vel.x = 255.0f;
            }
        }
...
void Macbeth_801A68F8(MaMechbeth* this, s16 arg1, f32 arg2, f32 arg3, f32 arg4, f32 arg5, f32 arg6, f32 arg7) {
...
void Macbeth_801A6FB4(MaMechbeth* this) {
    // FAKE
    //if (1) {}
...
void Macbeth_801A72DC(MaMechbeth* this) {
    Macbeth_801A0308(this, this->obj.pos.z, this->obj.rot.y, 0);
...
void Macbeth_801A74C4(MaMechbeth* this) {
    Macbeth_801A0308(this, this->obj.pos.z, this->obj.rot.y, 0);
...
void Macbeth_801A78B0(MaMechbeth* this) {
    Macbeth_801A0308(this, this->obj.pos.z, this->obj.rot.y, 0);
...
void Macbeth_801A7A30(MaMechbeth* this) {
    Macbeth_801A0308(this, this->obj.pos.z, this->obj.rot.y, 0);
...
void Macbeth_MaMechbeth_FacePlayer(MaMechbeth* this) {
    f32 xDist;
    f32 yDist;
    f32 xRot;
    f32 yRot;
...
void Macbeth_MaMechbeth_Init(MaMechbeth* this) {
    u8 i;
...
void Macbeth_MaMechbeth_Update(MaMechbeth* this) {
    s32 pad;
    f32 temp;
    f32 sp374;
    f32 var_fv0;
    ...
    switch (this->state) {
        ...
        case 2:
            ...
            if (this->timer_0BC == 0) {
                ...
                if ((this->fwork[5] + 500.0f) <= this->obj.pos.y) {
                    ...
                    this->info.hitbox = SEGMENTED_TO_VIRTUAL(aMaMechbethHitbox);
                    this->state = 3;
                }
            }
            Macbeth_8019A830(this);
        ...
        case 4:
            ...
            Macbeth_801A6984(this);
            Macbeth_MaMechbeth_FacePlayer(this);
        ...
        case 18:
            ...
            if (D_i5_801BE368[3] < ((gPlayer[0].trueZpos - this->obj.pos.z) * 0.2f)) {
                Math_SmoothStepToF(&D_i5_801BE368[2], 15.0f, 0.2f, 2.0f, 0.01f);
                D_i5_801BE368[3] += D_i5_801BE368[2];
...
//                if (1) {}
                gHitCount += 51;
                D_ctx_80177850 = 15;
                Effect_FireSmoke1_Spawn3(this->obj.pos.x, this->obj.pos.y, this->obj.pos.z, 20.0f);
                Macbeth_Effect357_Spawn1(this->vwork[2].x, this->vwork[2].y, this->vwork[2].z,
    ...
    if ((this->dmgType != DMG_NONE) && (this->state >= 2)) {
        ...
        if ((this->dmgPart == 0) && (D_i5_801BE320[9] > 0) && ((gPlayer[0].trueZpos - this->obj.pos.z) > 200.0f)) {
            ...
            if (D_i5_801BE320[9] <= 0) {
                AUDIO_PLAY_SFX(NA_SE_EN_EXPLOSION_M, this->sfxSource, 4);
                Effect_FireSmoke1_Spawn3(this->obj.pos.x, this->obj.pos.y + 80.0f, this->obj.pos.z + 50.0f, 5.0f);
                for (i = 0; i < 2; i++) {
                    Macbeth_Effect357_Spawn1(
            ...
            if (D_i5_801BE320[10] <= 0) {
                AUDIO_PLAY_SFX(NA_SE_EN_EXPLOSION_M, this->sfxSource, 4);
                Effect_FireSmoke1_Spawn3(this->obj.pos.x, this->obj.pos.y - 30.0f, this->obj.pos.z, 6.0f);
    ...
    if ((D_i5_801BE320[16] != 0) && (gPlayer[0].state != PLAYERSTATE_LEVEL_COMPLETE)) {
        if (gBossFrameCount == 0) {
            ...
            gShowBossHealth = 1;
            gBossHealthBar = (s32) ((D_i5_801BE320[29] * 0.34f));// / 250.f) * 85.0f);
            gBossHealthBar += (s32) ((D_i5_801BE320[9] * 0.425f));/// 200.f) * 85.0f);
            gBossHealthBar += (s32) ((D_i5_801BE320[10] * 0.34f)); /// 250.f) * 85.0f);
        }
        gBossFrameCount++;
    }
    ...
    switch (D_i5_801BE320[2]) {
        case 1:
            ...
            Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 0.2f, 100.0f, 0.01f);
            break;

        case 2:
            temp_a3 = Animation_GetFrameData(&D_MA_601D188, D_i5_801BE320[3], spD4);
            Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 0.15f, 100.0f, 0.01f);
            break;

        case 3:
            temp_a3 = Animation_GetFrameData(&D_MA_6017714, D_i5_801BE320[3], spD4);
            if (D_i5_801BE320[31] != 0) {
                Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 0.1f, 100.0f, 0.01f);
            } else {
                Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 1.0f, 100.0f, 0.01f);
            }
            break;

        case 4:
            temp_a3 = Animation_GetFrameData(&D_MA_6016B2C, D_i5_801BE320[3], spD4);
            Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 0.2f, 50.0f, 0.01f);
            break;
        ...
        case 6:
            ...
            Math_SmoothStepToVec3fArray(spD4, sMaMechbethJointTable, 1, temp_a3, 0.1f, 50.0f, 0.01f);
            break;
...
s32 Macbeth_MaMechbeth_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
...
void Macbeth_MaMechbeth_PostLimbDraw(s32 limbIndex, Vec3f* rot, void* thisx) {
    Vec3f sp2C = { 0.0f, 0.0f, 0.0f };
    MaMechbeth* this = (MaMechbeth*) thisx;
...
void Macbeth_MaMechbeth_Draw(MaMechbeth* this) {
    if (gPlayer[0].state == PLAYERSTATE_ACTIVE) {
        if (((gPlayer[0].trueZpos - this->obj.pos.z) > 7000.0f) ||
            ((gPlayer[0].trueZpos - this->obj.pos.z) < -1000.0f)) {
            return;
    ...
    if (D_i5_801BE320[2] == 0) {
        ...
        Animation_GetFrameData(&D_MA_601EA28, D_i5_801BE320[3], sMaMechbethJointTable);
    }
    Animation_DrawSkeleton(3, aMaMechbethSkel, sMaMechbethJointTable, Macbeth_MaMechbeth_OverrideLimbDraw,
                           Macbeth_MaMechbeth_PostLimbDraw, this, gCalcMatrix);
}
...
s32 Macbeth_MaTenderCar_OverrideLimbDraw(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* thisx) {
    RCP_SetupDL(&gMasterDisp, SETUPDL_29);
    gSPClearGeometryMode(gMasterDisp++, G_TEXTURE_GEN);
...
// unused
void Macbeth_CsGreatFox_Setup(ActorCutscene* this) {
    Actor_Initialize(this);
    this->obj.status = OBJ_INIT;
...
void Macbeth_LevelStart(Player* player) {
    ...
    switch (player->csState) {
        case 0:
            gLevelObjects = SEGMENTED_TO_VIRTUAL(aMaCsLongTrainObjects);
...
void Macbeth_LongTrainObjects(void) {
    ...
    gLevelObjects = SEGMENTED_TO_VIRTUAL(aMaLongTrainObjects);
...
void Macbeth_ShortTrainObjects(void) {
    ...
    gLevelObjects = SEGMENTED_TO_VIRTUAL(aMaShortTrainObjects);
                ...
                for (j = 0; j < temp_ft3; j++, hitboxData += 6) {
                    ...
                    if ((j == modFrames) && (hitboxData[1] > -100.0f) && (hitboxData[3] > -100.0f)) {
                        ...
                        if (VEC3F_MAG(&test) < radius) {
                            ...
                            if (actor->obj.id == OBJ_ACTOR_MA_MECHBETH) {
                                actor->damage = 4;
                            }
                        }
                    }
...
//                            var_v0 = 5;
                    ...
                    Math_SmoothStepToF(&this->obj.pos.x,
                                       gPlayer[0].pos.x + D_i5_801BA570[this->iwork[2]][this->iwork[1]] * recip_var_v0, 0.2f,
                                       30.0f, 0.01f);
                    Math_SmoothStepToF(&this->obj.pos.z,
                                       gPlayer[0].trueZpos - D_i5_801BA638[this->iwork[2]][this->iwork[1]] * recip_var_v0,
                                       0.2f, 40.0f, 0.01f);
...
void Macbeth_MaShockBox_Update(MaShockBox* this) {
    ...
    if ((this->dmgType != DMG_NONE) && (this->dmgPart == 0)) {
        ...
        this->timer_0C6 = 15;
        if (this->health <= 0) {
            Effect_TimedSfx_Spawn(&this->obj.pos, NA_SE_EXPLOSION_S);
...
void Macbeth_LevelComplete2(Player* player) {
    ...
    if (player->pathChangeTimer != 0) {
        ...
        Math_SmoothStepToF(&player->pathStep, gPathVelZ * 0.3f, 0.1f, 2.0f, 0.0001f);
